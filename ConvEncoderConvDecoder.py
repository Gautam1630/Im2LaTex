...
for i in range(0,4):
  res=next_layer
  W = weight_variable("enc_weight"+str(i), [3,3,128,256]) 
  b = bias_variable("enc_bias"+str(i), [256]) 
...
time=0
final_out=tf.zeros([batch_size,1],dtype=tf.int64) 
logits_for_softmax=tf.zeros([batch_size,1,vocab_size],dtype=tf.float32)
...
while(time < max_decode_length-1):
	...
	for i in range(0,3):
	  ...
	  Wd=tf.get_variable("wt"+str(i)+str(time),shape=[3,128,256],initializer=None,trainable=True) 
	  bd=tf.get_variable("biases"+str(i)+str(time),shape=[256],initializer=None,trainable=True) 
	...
	logits_for_softmax=tf.concat([logits_for_softmax,logits],1) 
	sample_ids = tf.argmax(logits, axis=-1) 
	next_inputs = tf.nn.embedding_lookup(embed, sample_ids)
	
	final_out=tf.concat([final_out,sample_ids],1) 

	next_inputs = tf.reshape(next_inputs, [batch_size, 1,128]) 
	...
	next_inputs.set_shape([batch_size, max_decode_length, 128]) 
	inputs=next_inputs
	time=time+1
